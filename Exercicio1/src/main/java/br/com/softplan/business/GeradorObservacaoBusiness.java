 ...
 * 
 */
public class GeradorObservacaoBusiness {
	...
	public GeradorObservacaoBusiness() {
		GeradorObservacaoBusiness.totalValores = 0.0;
		(GeradorObservacaoBusiness.moedaPtbr).setMinimumFractionDigits(2);
	}
	...
	public String geraObservacao(Map<Integer, String> notasFiscaisValores) {
		...
		if (hasNumerosNotasFiscais(notasFiscais)) {
			...
			return message + codigosConcatenados + ". Total = " + (GeradorObservacaoBusiness.moedaPtbr)
					.format(Double.valueOf(GeradorObservacaoBusiness.totalValores));
		}
		return "";
	}
	...
	public static StringBuilder retornaCodigos(Map<Integer, String> numerosNotasFiscais) {
		...
		numerosNotasFiscais.entrySet().forEach(e -> {
			GeradorObservacaoBusiness.totalValores += Double.valueOf(e.getValue());
			String valorPtBr = (GeradorObservacaoBusiness.moedaPtbr).format(Double.valueOf(e.getValue()));
	...
	private static class Constants {
		...
		public static final String TX_GERADOR_OBSERVACAO_CUJO_VALOR = " cujo valor é R$ ";
	}
